          ...
          return (
            <div key={i} onClick={() => { if (!isDisabled) onChange(dateStr); }}
              style={{
                ...
                color: isDisabled ? T.textMut + "44" : isSel ? "white" : isMatch ? (i % 7 === 0 ? "#ef4444" : i % 7 === 6 ? "#3b82f6" : T.text) : (i % 7 === 0 ? "#ef4444" : i % 7 === 6 ? "#3b82f6" : T.text),
...
export function RecurringForm({ type, initial, onSubmit, onCancel, T }) {
  ...
  const [monthlyMode, setMonthlyMode] = useState(initial?.monthlyMode || "day");
  const [nthWeek, setNthWeek] = useState(initial?.nthWeek || 1);
  const [nthDayOfWeek, setNthDayOfWeek] = useState(initial?.nthDayOfWeek ?? 1);
  ...
  return (
    <div>
      ...
      <label style={{ display: "block", fontSize: 14, fontWeight: 600, color: T.textSec, marginBottom: 6, marginTop: 16 }}>{type === "weekly" ? "요일" : "일자 지정 방식"}</label>
      {type === "weekly"
        ...
        : <>
          <div style={{ display: "flex", gap: 6, marginTop: 6, marginBottom: 12 }}>
            <button onClick={() => setMonthlyMode("day")} style={{ padding: "8px 16px", borderRadius: 8, border: "none", cursor: "pointer", fontSize: 14, fontWeight: 600, background: monthlyMode === "day" ? T.primary : T.surfaceBg, color: monthlyMode === "day" ? "white" : T.text, opacity: monthlyMode === "day" ? 1 : 0.5 }}>일자 지정</button>
            <button onClick={() => setMonthlyMode("nthWeekday")} style={{ padding: "8px 16px", borderRadius: 8, border: "none", cursor: "pointer", fontSize: 14, fontWeight: 600, background: monthlyMode === "nthWeekday" ? T.primary : T.surfaceBg, color: monthlyMode === "nthWeekday" ? "white" : T.text, opacity: monthlyMode === "nthWeekday" ? 1 : 0.5 }}>주차/요일 지정</button>
          </div>
          {monthlyMode === "day"
            ...
            : <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
                <span style={{ fontSize: 14, fontWeight: 600, color: T.textSec, flexShrink: 0 }}>매월</span>
                <select style={{ padding: "6px 10px", borderRadius: 6, border: `1.5px solid ${T.inputBorder}`, fontSize: 14, fontWeight: 600, background: T.surfaceBg, color: T.text, cursor: "pointer", outline: "none", height: 34 }} value={nthWeek} onChange={(e) => setNthWeek(Number(e.target.value))}>
                  {[1, 2, 3, 4].map((w) => <option key={w} value={w}>{w}째 주</option>)}
                </select>
                <div style={{ display: "flex", gap: 4 }}>{DAYS_KR.map((d, i) => <button key={i} onClick={() => setNthDayOfWeek(i)} style={{ padding: "6px 10px", borderRadius: 6, border: "none", cursor: "pointer", fontSize: 14, fontWeight: 600, height: 34, background: nthDayOfWeek === i ? T.primary : T.surfaceBg, color: nthDayOfWeek === i ? "white" : T.text }}>{d}</button>)}</div>
              </div>
          }
        </>}
      {type === "weekly" && <>
        ...
        <div style={{ flex: 1 }}>
          ...
          <div style={{ display: "flex", gap: 4, alignItems: "center" }}>
            <div onClick={() => setOpenCal(openCal === "end" ? null : "end")}
              ...
              <span style={{ fontSize: 14, color: endDate ? "#000" : T.textMut, fontWeight: endDate ? 600 : 400 }}>{fmtDisplay(endDate) || "없음"}</span>
              <span style={{ fontSize: 14, color: T.textMut }}>📅</span>
            </div>
      ...
      {openCal && (
        <div style={{ marginTop: 8 }}>
          <DayHighlightCalendar
            ...
            highlightDow={type === "weekly" ? dv : undefined}
      ...
      <div style={{ display: "flex", justifyContent: "flex-end", gap: 8, marginTop: 24 }}>
        <button style={{ padding: "10px 22px", border: `1px solid ${T.border}`, background: T.cardBg, borderRadius: 10, cursor: "pointer", fontSize: 15, fontWeight: 500, color: T.textSec }} onClick={onCancel}>취소</button>
        <button style={{ padding: "10px 22px", border: "none", background: `linear-gradient(135deg,${T.primary},${T.accent})`, color: "white", borderRadius: 10, cursor: "pointer", fontSize: 15, fontWeight: 600 }} onClick={() => n.trim() && onSubmit(n.trim(), dv, timeStr, intv, startDate, endDate, { monthlyMode, nthWeek, nthDayOfWeek })} disabled={!n.trim()}>{initial ? "저장" : "추가"}</button>
      </div>
    </div>
  );
}
...
export function QuickTaskForm({ initial, onSubmit, onCancel, T }) {
  const [n, sN] = useState(initial?.name || "");
  const [desc, sDesc] = useState(initial?.description || "");
  const parseTime = (t) => { if (!t) return { h: "", m: "" }; const [h, m] = t.split(":"); return { h: h || "", m: m || "" }; };
  const initT = parseTime(initial?.time || "");
  const initET = parseTime(initial?.endTime || "");
  const [hour, sH] = useState(initT.h);
  const [minute, sM] = useState(initT.m);
  const [endHour, sEH] = useState(initET.h);
  const [endMinute, sEM] = useState(initET.m);
  const timeStr = hour !== "" && minute !== "" ? `${hour}:${minute}` : "";
  const endTimeStr = endHour !== "" && endMinute !== "" ? `${endHour}:${endMinute}` : "";
  const inp = { width: "100%", padding: "12px 16px", border: `1.5px solid ${T.inputBorder}`, borderRadius: 10, fontSize: 16, outline: "none", background: T.surfaceBg, color: T.text };
  return (
    <div>
      <h3 style={{ fontSize: 20, fontWeight: 700, marginBottom: 20 }}>{initial ? "퀵 일정 편집" : "퀵 일정 추가"}</h3>
      <label style={{ display: "block", fontSize: 14, fontWeight: 600, color: T.textSec, marginBottom: 6 }}>이름</label>
      <input style={inp} value={n} onChange={(e) => sN(e.target.value)} autoFocus placeholder="예: 헬스, 독서" />
      <label style={{ display: "block", fontSize: 14, fontWeight: 600, color: T.textSec, marginBottom: 6, marginTop: 16 }}>설명 (선택)</label>
      <textarea style={{ ...inp, minHeight: 60, resize: "vertical" }} value={desc} onChange={(e) => sDesc(e.target.value)} placeholder="간단한 메모..." />
      <label style={{ display: "block", fontSize: 14, fontWeight: 600, color: T.textSec, marginBottom: 6, marginTop: 16 }}>시간 (선택)</label>
      <div style={{ display: "flex", gap: 8, alignItems: "center", marginTop: 4 }}>
        <select style={{ ...inp, flex: 1, cursor: "pointer" }} value={hour} onChange={(e) => sH(e.target.value)}><option value="">시</option>{Array.from({ length: 24 }, (_, i) => String(i).padStart(2, "0")).map((h) => <option key={h} value={h}>{h}시</option>)}</select>
        <span style={{ fontSize: 20, fontWeight: 700, color: T.textMut }}>:</span>
      ...
      {timeStr && <>
        <label style={{ display: "block", fontSize: 14, fontWeight: 600, color: T.textSec, marginBottom: 6, marginTop: 16 }}>종료 시간 (선택)</label>
        <div style={{ display: "flex", gap: 8, alignItems: "center", marginTop: 4 }}>
          <select style={{ ...inp, flex: 1, cursor: "pointer" }} value={endHour} onChange={(e) => sEH(e.target.value)}><option value="">시</option>{Array.from({ length: 24 }, (_, i) => String(i).padStart(2, "0")).map((h) => <option key={h} value={h}>{h}시</option>)}</select>
          <span style={{ fontSize: 20, fontWeight: 700, color: T.textMut }}>:</span>
          <select style={{ ...inp, flex: 1, cursor: "pointer" }} value={endMinute} onChange={(e) => sEM(e.target.value)}><option value="">분</option>{Array.from({ length: 12 }, (_, i) => String(i * 5).padStart(2, "0")).map((m) => <option key={m} value={m}>{m}분</option>)}</select>
          {(endHour !== "" || endMinute !== "") && <button style={{ width: 30, height: 30, border: "none", background: "transparent", cursor: "pointer", borderRadius: 7, fontSize: 13, color: T.textMut }} onClick={() => { sEH(""); sEM(""); }}>초기화</button>}
        </div>
      </>}
      <div style={{ display: "flex", justifyContent: "flex-end", gap: 8, marginTop: 24 }}>
        <button style={{ padding: "10px 22px", border: `1px solid ${T.border}`, background: T.cardBg, borderRadius: 10, cursor: "pointer", fontSize: 15, fontWeight: 500, color: T.textSec }} onClick={onCancel}>취소</button>
        <button style={{ padding: "10px 22px", border: "none", background: `linear-gradient(135deg,${T.primary},${T.accent})`, color: "white", borderRadius: 10, cursor: "pointer", fontSize: 15, fontWeight: 600 }} onClick={() => n.trim() && onSubmit(n.trim(), desc.trim(), timeStr, endTimeStr)} disabled={!n.trim()}>{initial ? "저장" : "추가"}</button>
      </div>
    </div>
  );
}
